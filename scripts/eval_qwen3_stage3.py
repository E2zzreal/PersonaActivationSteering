...
def main():
    ...
    # ✅ 修正：使用 Stage 3 checkpoint
    checkpoint_path = "checkpoints/stage3_qwen3/best.pt"
    base_model_path = "/home/kemove/.cache/modelscope/Qwen/Qwen3-4B"
    
    logger.info(f"\n{'='*60}")
    logger.info(f"Evaluating Stage 3 (Qwen3-4B Model Generation)")
    ...
    # 保存生成的回复
    with open(f"results/qwen3_stage3_responses.json", "w") as f:
    ...
    with open("results/qwen3_stage3_eval.json", "w") as f:
        json.dump(output, f, indent=2, ensure_ascii=False)
    
    logger.info(f"\nResults saved to results/qwen3_stage3_eval.json")

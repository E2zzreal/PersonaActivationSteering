"""
PersonaSteer模型生成回复评估 - 修复版
关键修复：为HyperNetwork设置tokenizer
"""
...
5=Perfectly aligned"""
    ...
    def generate_response(self, model, tokenizer, user_input: str, 
        ...
        try:
            # 使用模型generate
            with torch.no_grad():
                # 核心修复：传入personality作为user_texts
                generated_ids, v_t = model.generate(
                    input_ids=input_ids,
                    v_prev=v_prev,
                    user_texts=[personality],  # ← 关键：personality编码
                    max_new_tokens=128,
                    temperature=0.7,
                )
    ...
    def evaluate_stage(self, checkpoint_path: str, base_model_path: str,
        ...
        for sample in tqdm(eval_samples[:num_samples], desc=f"Stage {stage}"):
            ...
            for turn in conversations[:3]:  # 减少到3轮以加快速度

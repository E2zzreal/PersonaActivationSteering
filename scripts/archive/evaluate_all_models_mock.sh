...
LOG_FILE="logs/eval_all_models_mock_${TIMESTAMP}.log"

echo "========================================" | tee -a "$LOG_FILE"
echo "模拟LLM Judge评估 - ${TIMESTAMP}" | tee -a "$LOG_FILE"
echo "（基于关键词匹配，无需API）" | tee -a "$LOG_FILE"
echo "========================================" | tee -a "$LOG_FILE"
...
evaluate_model() {
    ...
    if [ ! -f "$conv_file" ]; then
        echo "[$name] ✗ 对话文件不存在" | tee -a "$LOG_FILE"
        return 1
    fi

    echo "[$name] 开始评估..." | tee -a "$LOG_FILE"

    python scripts/mock_llm_judge_eval.py \
...
eval_files = sorted(glob.glob("results/judge_eval_*_20260410_*.json"))
...
if not eval_files:
    ...
    for eval_file in eval_files:
        name = Path(eval_file).stem.replace("judge_eval_", "").replace("_20260410_", " ").split()[0]
        try:
    ...
    print(f"\n汇总已保存: results/evaluation_summary.json")

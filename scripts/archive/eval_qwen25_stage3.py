...
def main():
    ...
    base_model_path = "/home/kemove/.cache/modelscope/Qwen/Qwen2.5-3B"
    
    logger.info(f"\n{'='*60}")
    logger.info(f"Evaluating Stage 3 (Qwen2.5-3B Model Generation)")
    logger.info(f"Checkpoint: {checkpoint_path}")
    logger.info(f"{'='*60}")
    
    results, responses = evaluator.evaluate_stage(
        checkpoint_path, base_model_path, eval_samples, args.device, args.num_samples
    )
    ...
    # 保存
    with open(f"results/qwen25_stage3_responses.json", "w") as f:
    ...
    with open("results/qwen25_stage3_eval.json", "w") as f:
        json.dump(output, f, indent=2, ensure_ascii=False)
    
    logger.info(f"\nResults saved to results/qwen25_stage3_eval.json")

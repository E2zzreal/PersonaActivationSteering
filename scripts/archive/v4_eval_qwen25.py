#!/usr/bin/env python
"""
V4 完整评估脚本 - Qwen2.5-3B 版本
评估 baseline + Stage1 + Stage2 + Stage3
"""
...
def load_persona_steer(checkpoint_path: str, base_model_path: str, device: str,
    ...
    # 默认 inject_layers: Qwen2.5-3B 的 [10-17]
    if inject_layers is None:
        inject_layers = list(range(10, 18))
...
def generate_baseline(model, tokenizer, user_input: str, personality: str,
    ...
    messages = [
        {"role": "system", "content": f"你的人格特征是：{personality}\n\n你的个人简介：{profile}"},
...
def main():
    ...
    parser.add_argument("--base_model", type=str,
                        default="/home/kemove/.cache/modelscope/Qwen/Qwen2.5-3B")
    ...
    parser.add_argument("--stage2_checkpoint", type=str,
                        default="checkpoints/v4_qwen25_stage2/best.pt")
    parser.add_argument("--stage3_checkpoint", type=str,
                        default="checkpoints/v4_qwen25_stage3/best.pt")
    ...
    stage_defs = {
        ...
        "stage1": {
            ...
            "inject_layers": list(range(10, 18)),  # Qwen2.5 Stage1 用 8 层
        },
        "stage2": {
            "type": "persona_steer",
            "checkpoint": args.stage2_checkpoint,
            "inject_layers": list(range(10, 18)),  # Qwen2.5 Stage2 同 8 层
        },
        "stage3": {
            "type": "persona_steer",
            "checkpoint": args.stage3_checkpoint,
            "inject_layers": list(range(10, 18)),  # Qwen2.5 Stage3 同 8 层
        },
    }
    ...
    # 打印对比表
    logger.info(f"\n{'='*60}")
    logger.info("Qwen2.5-3B EVALUATION SUMMARY")
    logger.info(f"{'='*60}")

"""
PersonaSteer完整评估 - 使用模型生成回复
关键：让Stage 1/2/3各自生成回复，然后评估
"""
...
5=Perfectly aligned"""
    ...
    def generate_response(self, model, tokenizer, user_input: str, 
                         personality: str, profile: str, device: str) -> str:
        """使用模型生成回复"""
        # 编码输入
        ...
        try:
            ...
            with torch.no_grad():
                generated_ids, v_t = model.generate(
                    input_ids=input_ids,
                    v_prev=v_prev,
                    user_texts=[personality],  # personality作为user_texts
                    max_new_tokens=128,
                    temperature=0.7,
                )
            ...
            for turn in conversations[:6]:
                ...
                if user_input and personality:
                    ...
                    response = self.generate_response(
                        model, tokenizer, user_input, personality, profile, device
                    )
...
def main():
    ...
    for stage_name, ckpt_path in checkpoints.items():
        logger.info(f"\n{'='*60}")
        logger.info(f"Evaluating {stage_name} (Model Generation)")
        logger.info(f"{'='*60}")
        ...
        # 保存生成的回复
        with open(f"results/{stage_name.lower().replace(' ', '_')}_responses.json", "w") as f:
            json.dump(responses, f, indent=2, ensure_ascii=False)
    
    # 保存完整结果
    with open("results/model_generation_eval.json", "w") as f:
        json.dump(all_results, f, indent=2, ensure_ascii=False)
    
    # 打印报告
    print("\n" + "="*60)
    print("MODEL GENERATION EVALUATION RESULTS")
    print("="*60)
